...
class HalfCheetahDirSparseEnv(HalfCheetahEnv):
    """Half-cheetah environment with target direction, as described in [1]. The
    code is adapted from
    https://github.com/cbfinn/maml_rl/blob/9c8e2ebd741cb0c7b8bf2d040c4caeeb8e06cc95/rllab/envs/mujoco/half_cheetah_env_rand_direc.py
    ...
    This is also a sparse task. The reward is given after xpos > 5.
    ...
    def step(self, action):
        ...
        if np.abs(forward_vel) > 0.8:
            print(forward_vel)
        sparse_vel = 1.0
        observation = self._get_obs()
        if (self._goal_dir == +1 and forward_vel > +sparse_vel) or (self._goal_dir == -1 and forward_vel < -sparse_vel):
