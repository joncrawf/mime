...
class E_MAMLTRPO(GradientBasedMetaLearner):
    ...
    def __init__(self,
                 ...
                 first_order=False,
                 device='cpu'):
        super(E_MAMLTRPO, self).__init__(policy, device=device)
    ...
    async def surrogate_loss(self, train_futures, valid_futures, old_pi=None, old_train_pis=None):
        ...
        with torch.set_grad_enabled(old_pi is None):
            ...
            params = None
            if old_train_pis is None:
                old_train_pis = [None for _ in train_futures]
            for i, futures in enumerate(train_futures):
                train_episodes = await futures

                train_pi = self.policy(train_episodes.observations, params=params)

                if old_train_pis[i] is None:
                    old_train_pis[i] = detach_distribution(train_pi)

                train_log_ratio = (train_pi.log_prob(train_episodes.actions)
                                   - old_train_pis[i].log_prob(train_episodes.actions))
                train_ratio = torch.exp(train_log_ratio)
        ...
        return losses.mean(), kls.mean(), old_pi, old_train_pis
    ...
    def step(self,
        ...
        old_losses, old_kls, old_pis, old_train_pis = self._async_gather([
        ...
        for _ in range(ls_max_steps):
            ...
            losses, kls, _, _ = self._async_gather([
                self.surrogate_loss(train, valid, old_pi=old_pi, old_train_pis=old_train_pi)
                for (train, valid, old_pi, old_train_pi)
                in zip(zip(*train_futures), valid_futures, old_pis, zip(*old_train_pis))])
