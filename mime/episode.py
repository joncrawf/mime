...
from mime.utils.torch_utils import weighted_normalize
...
class BatchEpisodes(object):
    ...
    def intrinsic_rewards(self):
        if self._intrinsic_rewards is None:
            raise AttributeError("No Intrinsic Rewards Available")
        return self._intrinsic_rewards

    @property
    def extrinsic_rewards(self):
        if self._extrinsic_rewards is None:
            if self._intrinsic_rewards is None:
                return self.rewards
        return self._extrinsic_rewards
